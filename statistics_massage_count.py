...
# 養生館人數統計 2019.12.06
class StatisticsMassageCount(QtWidgets.QMainWindow):
    # 初始化
    def __init__(self, parent=None, *args):
        super(StatisticsMassageCount, self).__init__(parent)
        self.parent = parent
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_MASSAGE_COUNT, self)
        system_utils.set_css(self, self.system_settings)
        self.table_widget_massage_count = table_widget.TableWidget(self.ui.tableWidget_massage_count, self.database)
    ...
    def _set_table_width(self):
        ...
        self.table_widget_massage_count.set_table_heading_width(width)
    ...
    def start_calculate(self):
        self.ui.tableWidget_massage_count.setRowCount(0)
    ...
    def _set_statistics_table_heading(self):
        ...
        self.ui.tableWidget_massage_count.setRowCount(row_count + 1)

        for row_no, case_date in zip(range(row_count), calendar_list):
            self.ui.tableWidget_massage_count.setItem(
    ...
    def _calculate_data(self):
        ...
        self._calculate_purchase_count(rows)
    ...
    def _reset_data(self):
        ...
        for row_no in range(self.ui.tableWidget_massage_count.rowCount()):
            for col_no in range(1, self.ui.tableWidget_massage_count.columnCount()):
                self.ui.tableWidget_massage_count.setItem(
                    row_no, col_no, QtWidgets.QTableWidgetItem('0')
                )
                self.ui.tableWidget_massage_count.item(
                    row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
                if col_no in [4, 8, 9]:
                    self.ui.tableWidget_massage_count.item(row_no, col_no).setFont(font)
    ...
    def _read_data(self):
        ...
        sql = '''
            SELECT  
                MassageCaseKey, CaseDate, Period, TreatType
            FROM massage_cases
            WHERE
    ...
    def _get_row_no(self, case_date):
        for row_no in range(self.ui.tableWidget_massage_count.rowCount()):
            case_date_field = self.ui.tableWidget_massage_count.item(row_no, 0)
    ...
    def _calculate_massage_count(self, rows):
        ...
        total_row_no = self.ui.tableWidget_massage_count.rowCount() - 1
        ...
        for row in rows:
            ...
            case_count = number_utils.get_integer(
                self.ui.tableWidget_massage_count.item(row_no, col_no).text()
            ) + 1
            subtotal = number_utils.get_integer(
                self.ui.tableWidget_massage_count.item(row_no, subtotal_col_no).text()
            ) + 1

            total = number_utils.get_integer(
                self.ui.tableWidget_massage_count.item(total_row_no, col_no).text()
            ) + 1

            self._set_item_data(row_no, col_no, string_utils.xstr(case_count))
        ...
        total = (
                number_utils.get_integer(self.ui.tableWidget_massage_count.item(
    ...
    def _calculate_purchase_count(self, rows):
        period_col_no = {
            '早班': 5,
            '午班': 6,
            '晚班': 7,
    ...
    def _calculate_subtotal(self):
        for row_no in range(self.ui.tableWidget_massage_count.rowCount()):
            massage_subtotal = number_utils.get_integer(self.ui.tableWidget_massage_count.item(row_no, 4).text())
            purchase_subtotal = number_utils.get_integer(self.ui.tableWidget_massage_count.item(row_no, 8).text())
    ...
    def export_to_excel(self):
        ...
        export_utils.export_table_widget_to_excel(
            excel_file_name, self.ui.tableWidget_massage_count, None,
            [1, 2, 3, 4, 5, 6, 7, 8, 9],
        )
    ...
    def _plot_massage_count_chart(self):
        ...
        for i in range(len(massage_type)):
            ...
            set_list[i] << number_utils.get_integer(
                self.ui.tableWidget_massage_count.item(
                    self.ui.tableWidget_massage_count.rowCount()-1, massage_col_no[i]).text()
            )
