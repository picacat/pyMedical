...
# 2018.07.09
class PrintCertificatePaymentPrescript:
    # 初始化
    ...
    def _get_html_title(self, row):
        html = '''
            <h1 style="text-align: center">{clinic_name} 醫療費用證明書</h1>
    ...
    def _get_fees_detail(self, ins_type):
        ...
        for row_no, row in zip(range(1, len(rows)+1), rows):
            ...
            prescript_html = self._get_prescript_html(row['CaseKey'], ins_type)
            html += '''
                <tr>
    ...
    def _get_prescript_html(self, case_key, ins_type):
        html = ''
        ...
        medicine_set_script = ''
        if ins_type in ['健保']:
            medicine_set_script = 'MedicineSet = 1 AND'
        ...
        '''.format(
            medicine_set_script=medicine_set_script,
            case_key=case_key,
        )
        rows = self.database.select_record(sql)
