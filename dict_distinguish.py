...
class DictDistinguish(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_dict_distinguish = table_widget.TableWidget(self.ui.tableWidget_dict_distinguish, self.database)
    ...
    def _set_dict_distinguish_data(self, rec_no, rec):
        ...
        for column in range(0, self.ui.tableWidget_dict_distinguish.columnCount()):
            self.ui.tableWidget_dict_distinguish.setItem(rec_no, column, QtWidgets.QTableWidgetItem(dict_distinguish_rec[column]))
    ...
    def _remove_distinguish(self):
        msg_box = dialog_utils.get_message_box(
            ...
            '<font size="4" color="red"><b>確定刪除{0}: "{1}"?</b></font>'.format(
                self.dict_type, self.table_widget_dict_distinguish.field_value(3)),
            '注意！資料刪除後, 將無法回復!'
        )
        remove_record = msg_box.exec_()
        if not remove_record:
