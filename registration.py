...
class Registration(QtWidgets.QMainWindow):
    ...
    def _check_first_visit_reservation(self, ic_card):
        ...
        if len(rows) > 0:
            ...
            if arrival:
                ...
                self.parent.open_reservation(reserve_key, None, None)
    ...
    def _selection_changed(self, i):
        ...
        if sender_name == 'comboBox_patient_share':
            ...
            # database.ui.comboBox_room.setCurrentText(string_utils.xstr(room))
    ...
    def _modify_wait(self):
        ...
        if tab_name == '候診名單':
            ...
            in_use = self.ui.tableWidget_wait.cellWidget(self.ui.tableWidget_wait.currentRow(), 2)
        else:
        ...
        if in_use:
            system_utils.show_message_box(
                QMessageBox.Warning,
                '無法修正掛號資料',
                '<font size="4" color="red"><b>此筆資料正在看診中, 請於看診後再修正.</b></font>',
                '請於資料看診完畢後再至已就診名單修正.'
            )
            return
        ...
        self.ui.groupBox_registration.setTitle('掛號資料-{0}'.format(case_key))
    ...
    def _check_ic_card_basic_data(self, ic_card, row):
        if ic_card is None:
            return
    ...
    def _prepare_registration_data(self, row, ic_card=None):
        ...
        if not self._check_deposit(patient_key, ic_card):
            self.parent.open_return_card(patient_key)
            return False
    ...
    def _completion_course(self, patient_key):
        if self.ui.comboBox_reg_type.currentText() in nhi_utils.TOUR_TYPE:
            return

        today = datetime.date.today()
        last_treat_date = (today - datetime.timedelta(days=30-1)).strftime('%Y-%m-%d 00:00:00')
        sql = '''
            SELECT CaseDate, TreatType, Card, Continuance, Share, Injury, XCard FROM cases WHERE
            (CaseDate >= "{0}") AND
            (PatientKey = {1}) AND
            (InsType = "健保") 
    ...
    def _registration_precheck(self, patient_key):
        ...
        message = registration_utils.check_deposit(             # 檢查健保欠卡未還
    ...
    def _check_deposit(self, patient_key, ic_card):
        message = registration_utils.check_deposit(
            self.database, self.system_settings, patient_key
        )  # 檢查欠卡
        if ic_card is not None and message is not None:  # 讀健保卡且上次未還卡
            ...
            msg_box.setInformativeText("請注意！如果欠卡日期為至 [健保卡欠還卡]作業執行[健保還卡]後，再繼續掛號!")
            msg_box.addButton(QPushButton("確定"), QMessageBox.YesRole)
            msg_box.exec_()
            self._cancel_registration()
            return False
    ...
    def _get_share_type(self, reg_type):
        ...
        if share_type == '基層醫療':
            try:
                age = number_utils.get_integer(self.ui.lineEdit_age.text().split('歲')[0])
            except:
                age = None
    ...
    def _get_reg_type(self, reg_type):
        if reg_type == '':
            reg_type = '一般門診'
    ...
    def _set_registration_data(self, patient_key, medical_record=None):
        ...
        self.ui.comboBox_tour_area.setCurrentText(tour_area)
        self.ui.comboBox_injury_type.setCurrentText(injury_type)
        self.ui.comboBox_treat_type.setCurrentText(treat_type)
    ...
    def delete_wait_list(self, skip_warning=None):
        ...
        if not skip_warning:
            ...
            msg_box.setInformativeText("注意！病歷資料也會一併刪除, 且無法回復!")
            msg_box.addButton(QPushButton("取消"), QMessageBox.NoRole)
            msg_box.addButton(QPushButton("確定"), QMessageBox.YesRole)
        ...
        card = self.table_widget_wait.field_value(12)
        course = self.table_widget_wait.field_value(13)
        log = '{patient_name}於{now}執行掛號刪除, 卡序:{card}, 主治醫師: {room}診{doctor}醫師'.format(
            patient_name=name,
            now=date_utils.now_to_str(),
            ins_type=self.table_widget_wait.field_value(7),
            card=card + '-{0}'.format(course) if number_utils.get_integer(course) >= 1 else card,
            room=self.table_widget_wait.field_value(14),
            doctor=self.table_widget_wait.field_value(15),
        )
    ...
    def _verify_registration_data(self, card):
        ...
        if self.ui.comboBox_reg_type.currentText() in nhi_utils.TOUR_TYPE:  # 巡迴醫療部檢查
            return True
    ...
    def _insert_medical_record(self, ic_card=None):
        ...
        log = '{patient_name}於{now}完成{ins_type}掛號, 卡序:{card}, 主治醫師: {room}診{doctor}醫師'.format(
        ...
        if self.ui.lineEdit_regist_fee.text() != '0':
            log += ', 掛號費: {0}'.format(self.ui.lineEdit_regist_fee.text())
        if self.ui.lineEdit_diag_share_fee.text() != '0':
            log += ', 門診負擔: {0}'.format(self.ui.lineEdit_diag_share_fee.text())
        if self.ui.lineEdit_deposit_fee.text() != '0':
            log += ', 欠卡費: {0}'.format(self.ui.lineEdit_deposit_fee.text())

        self._write_event_log('系統操作', log)
    ...
    def _update_medical_record(self):
        fields = [
            'Name', 'Visit', 'RegistType', 'TourArea', 'Injury',
            'TreatType', 'Share', 'InsType', 'Card', 'Continuance', 'XCard', 'Period',
            'Room', 'RegistNo', 'Massager', 'Register', 'DesignatedDoctor', 'DesignatedMassager',
            'ApplyType', 'PharmacyType',
        ...
        log = '{patient_name}於{now}執行掛號修正, 卡序:{card}, 主治醫師: {room}診{doctor}醫師'.format(
            patient_name=self.ui.lineEdit_name.text(),
            now=date_utils.now_to_str(),
            ins_type=self.ui.comboBox_ins_type.currentText(),
            card=card + '-{0}'.format(course) if number_utils.get_integer(course) >= 1 else card,
        ...
        self._write_event_log('資料修正', log)
    ...
    def _reservation(self):
        self.parent.open_reservation(None, None, None)
