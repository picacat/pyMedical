...
class DictCure(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_settings.load_ui_file(ui_settings.UI_DICT_CURE, self)
        ...
        self.table_widget_dict_cure = table_widget.TableWidget(self.ui.tableWidget_dict_cure, self.database)
    ...
    def dict_groups_name_changed(self):
        ...
        self.ui.groupBox_dict_cure.setTitle(
    ...
    def _remove_cure(self):
        msg_box = dialog_utils.get_message_box(
            ...
            '<font size="4" color="red"><b>確定刪除{0}: "{1}"?</b></font>'.format(
                self.dict_type, self.table_widget_dict_cure.field_value(3)),
            '注意！資料刪除後, 將無法回復!'
        )
        remove_record = msg_box.exec_()
        if not remove_record:
